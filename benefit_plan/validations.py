...
def validate_bf_unique_code(code, uuid=None):
    ...
    if instance:
        return [{"message": _("benefit_plan.validations.benefit_plan.code_exists" % {
            'code': code
        })}]
    return []
...
def validate_bf_unique_name(name, uuid=None):
    ...
    if instance:
        return [{"message": _("benefit_plan.validations.benefit_plan.name_exists" % {
            'name': name
        })}]
    return []
...
def validate_not_empty_field(string, field):
    if not string:
        return [{"message": _("benefit_plan.validations.field_empty") % {
            'field': field
        }}]
    return []
...
def validate_project_unique_name(name, benefit_plan_id, uuid=None):
    ...
    if instance:
        return [{"message": _("benefit_plan.validations.project.name_exists" % {
            'name': name
        })}]
    return []
